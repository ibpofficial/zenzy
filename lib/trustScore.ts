...
export function calculateTrustScore(input: {
  ...
  if (identityVerified) {
    ...
    const aadharStatus = (profile.documentVerifications as any)?.aadharStatus || (profile as any).aadharStatus;
    const panStatus = (profile.documentVerifications as any)?.panStatus || (profile as any).panStatus;

    if (aadharStatus === "approved") {
      identityVerificationScore += 10;
    } else if (aadharStatus === "pending" || !!((profile.documentVerifications as any)?.aadharDoc || (profile as any).aadharDoc)) {
      identityVerificationScore += 5;
    }

    if (panStatus === "approved") {
      identityVerificationScore += 10;
    } else if (panStatus === "pending" || !!((profile.documentVerifications as any)?.panDoc || (profile as any).panDoc)) {
      identityVerificationScore += 5;
    }
  }
  ...
  const gstStatus = (profile.documentVerifications as any)?.gstStatus;
  const licenseStatus = (profile.documentVerifications as any)?.licenseStatus;

  if (profile.verifiedBadges?.gst || gstStatus === "approved") {
    professionalDocumentsScore += 7;
  } else if (gstStatus === "pending" || !!(profile.documentVerifications as any)?.gstDoc) {
    professionalDocumentsScore += 3;
  } else if (profile.gstNumber || profile.documentVerifications?.gstNumber) {
    professionalDocumentsScore += 2;
  }

  if (profile.verifiedBadges?.businessReg || licenseStatus === "approved") {
    professionalDocumentsScore += 8;
  } else if (licenseStatus === "pending" || !!(profile.documentVerifications as any)?.licenseDoc) {
    professionalDocumentsScore += 3;
  } else if (profile.licenseNumber || profile.documentVerifications?.licenseNumber) {
    professionalDocumentsScore += 2;
  }

  if (profile.verifiedBadges?.officeAddress) {
    professionalDocumentsScore = Math.min(15, professionalDocumentsScore + 3);
  }
  ...
  const hasSocials = profile.socialLinks && Object.values(profile.socialLinks).some((val) => typeof val === "string" && val.trim().length > 0);
