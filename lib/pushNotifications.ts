...
export async function subscribeToPush(userId: string): Promise<boolean> {
  try {
    ...
    // Proactively register or get existing SW registration
    ...
    // Persist subscription to Firestore supporting multiple devices & domains (zenzy.shop & vercel)
    const subJson = subscription.toJSON();
    const endpoint = subJson.endpoint;

    if (endpoint && subJson.keys) {
      const endpointId = btoa(endpoint).replace(/[^a-zA-Z0-9]/g, "").slice(-30);
      const subDocRef = doc(db, "pushSubscriptions", userId);
      const subDocSnap = await getDoc(subDocRef).catch(() => null);
      let subscriptionsMap: Record<string, any> = {};
      if (subDocSnap && subDocSnap.exists()) {
        subscriptionsMap = subDocSnap.data()?.subscriptionsMap || {};
      }

      subscriptionsMap[endpointId] = {
        endpoint: subJson.endpoint,
        keys: subJson.keys,
        domain: typeof window !== "undefined" ? window.location.host : "zenzy.shop",
