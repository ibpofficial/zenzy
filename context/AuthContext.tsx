...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // Register Service Worker for PWA downloadability & background notifications
  useEffect(() => {
  ...
  // Automatic Trust Score Recalculation Check (Runs on mount, calls API endpoint which enforces 24h limit)
  useEffect(() => {
    const triggerRecalculate = async () => {
      try {
        ...
        console.error("Error triggering automatic trust score recalculation:", err);
      }
    };

    // Delay slightly to prevent competing with initial boot resources
    const timer = setTimeout(triggerRecalculate, 5000);
    return () => clearTimeout(timer);
  }, []);

  // Request Notification Permissions & subscribe to Web Push when user is authenticated
  useEffect(() => {
    if (typeof window !== "undefined" && "Notification" in window && user) {
      const initPush = async () => {
        try {
          ...
          console.warn("Push subscription init warning:", err);
        }
      };
      initPush();
    }
  ...
  const updateProfileImage = async (file: File): Promise<string> => {
    ...
    // Stage 1: Upload to Storage or generate compressed base64 fallback URL
    const avatarUrl = await uploadProfileImage(file, user.uid);
    const isBase64Fallback = !avatarUrl.startsWith("http");

    // Stage 2: Persist avatar to Firestore document using setDoc merge across relevant collections
    const updatePayload = {
      avatar: avatarUrl,
      avatarStorageType: isBase64Fallback ? "base64" : "url",
      updatedAt: new Date().toISOString()
    };

    // Always update users collection
    await setDoc(doc(db, "users", user.uid), updatePayload, { merge: true }).catch((err) =>
      console.error("Error setting avatar in users:", err)
    );

    // If role is worker or admin, update workers and admins collections as well
    if (role === "worker" || role === "admin") {
      await setDoc(doc(db, "workers", user.uid), updatePayload, { merge: true }).catch((err) =>
        console.error("Error setting avatar in workers:", err)
      );
    }
    if (role === "admin") {
      await setDoc(doc(db, "admins", user.uid), updatePayload, { merge: true }).catch((err) =>
        console.error("Error setting avatar in admins:", err)
      );
    }

    // Stage 3: Sync Firebase Auth photoURL if hosted HTTP URL
    try {
    ...
    // Stage 4: Update in-memory userData state instantly
    setUserData((prev: any) => (prev ? { ...prev, avatar: avatarUrl } : { uid: user.uid, avatar: avatarUrl }));
