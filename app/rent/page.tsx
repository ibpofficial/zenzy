...
export default function RentPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-b from-gray-50/50 via-white to-gray-50/30 font-['Inter',-apple-system,BlinkMacSystemFont,'Segoe_UI',Roboto,Helvetica,Arial,sans-serif] text-gray-900 antialiased overflow-x-hidden">
      <Navbar />

      <main className="max-w-7xl mx-auto w-full px-6 sm:px-10 pt-24 pb-16 flex-grow relative">
        {/* HERO SECTION */}
        ...
        {/* ── TOP END-TO-END OVERLAY: 4 DISTINCT CRISS-CROSSING SLOW CAUTION MARQUEES ── */}
        <div className="relative w-[100vw] left-1/2 -translate-x-1/2 overflow-hidden py-4 select-none z-40 opacity-95 pointer-events-none my-6">
          {[
            { angle: "-rotate-6 sm:-rotate-7", speed: "65s", reverse: false, bg: "bg-yellow-400 text-black border-black/80 shadow-2xl" },
            { angle: "rotate-5 sm:rotate-6", speed: "55s", reverse: true, bg: "bg-amber-400 text-slate-950 border-black/80 shadow-2xl" },
            { angle: "-rotate-4 sm:-rotate-5", speed: "72s", reverse: false, bg: "bg-yellow-300 text-black border-black/80 shadow-2xl" },
            { angle: "rotate-7 sm:rotate-8", speed: "60s", reverse: true, bg: "bg-amber-300 text-slate-950 border-black/80 shadow-2xl" },
          ].map((tape, idx) => {
            const cautionStr = "⚠️ CAUTION: PAGE IS UNDER CONSTRUCTION AND DEVELOPMENT 🚧 UNDER ACTIVE DEVELOPMENT 🛑 DO NOT PROCEED WITHOUT PERMISSION ⚠️";
            const fullLine = Array(6).fill(cautionStr).join("   •   ");
            const animClass = tape.reverse ? "animate-marquee-reverse" : "animate-marquee";
            return (
              <div
                key={idx}
                className={`transform ${tape.angle} ${tape.bg} font-black text-base sm:text-lg md:text-xl uppercase tracking-widest py-3 sm:py-3.5 border-y-2 border-dashed shadow-2xl overflow-hidden flex whitespace-nowrap -my-2.5`}
              >
                <div
                  className={`${animClass} flex shrink-0 items-center gap-8`}
                  style={{ animationDuration: tape.speed }}
                >
                  <span className="drop-shadow-md">{fullLine}</span>
                  <span className="drop-shadow-md">{fullLine}</span>
                </div>
                <div
                  className={`${animClass} flex shrink-0 items-center gap-8`}
