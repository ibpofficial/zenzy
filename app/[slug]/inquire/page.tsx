...
export default function ProfileInquiryPage() {
  ...
  const [wizardBudget, setWizardBudget] = useState("₹50,000 - ₹1,50,000");
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const validFiles = files.filter((file) => {
      ...
      const limit = isImg ? MAX_IMAGE_SIZE : MAX_DOC_SIZE;
      if (file.size > limit) {
        oversizedFiles.push(`• ${file.name} (${(file.size / (1024 * 1024)).toFixed(1)}MB, max ${(limit / (1024 * 1024))}MB)`);
        return false;
      }
      return true;
    });
    ...
    if (oversizedFiles.length > 0) {
      alert(`⚠️ The following file(s) exceed the size limit:\n\n${oversizedFiles.join("\n")}\n\nPlease use a smaller file or paste a Google Drive link below.`);
    }
  ...
  const handleSubmitInquiry = async () => {
    ...
    try {
      ...
      const allDocs = [...wizardPhotos];
      if (driveLink.trim()) {
        allDocs.push(driveLink.trim());
      }
      ...
      const payload = {
        ...
        requirements: wizardProblemDesc || "No special requirements detailed.",
        budgetRange: wizardBudget || worker.pricingRate || worker.pricing || "Flexible Budget",
        timelineEstimate: "Flexible Timeline",
        documents: allDocs,
        communicationSummary: wizardPhone,
        stage: "received" as const,
        stageHistory: [
          {
            stage: "received" as const,
            timestamp,
            note: "Inquiry submitted by client via profile wizard.",
            updatedBy: userData?.name || "Customer"
          }
        ],
        quotationIds: [],
        createdAt: timestamp,
      ...
      // Trigger notification using the notifications library
      const { triggerNotification } = await import("@/lib/notifications");
      await triggerNotification(
        worker.id,
        "New Project Inquiry",
        `Client ${userData?.name || "User"} sent you a new project inquiry: "${selectedService}"`,
        "system"
      );
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-20">
        <div className="w-full max-w-6xl">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-8 bg-white rounded-3xl shadow-2xl border border-slate-100 overflow-hidden">
                ...
                <div className="space-y-5 animate-in slide-in-from-right-4 duration-300">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between items-center">
                        <label className="text-xs font-medium text-slate-600">Estimated Project Budget <span className="text-red-500">*</span></label>
                        <span className="text-[10px] text-slate-400 font-semibold">Target Spending Limit</span>
                      </div>
                      <div className="flex flex-wrap gap-1.5 mb-2">
                        {["₹25,000 - ₹50,000", "₹50,000 - ₹1,50,000", "₹1,50,000 - ₹5,00,000", "₹5,00,000+"].map((bPreset) => (
                          <button
                            key={bPreset}
                            type="button"
                            onClick={() => setWizardBudget(bPreset)}
                            className={`px-3 py-1.5 rounded-lg text-xs font-extrabold transition cursor-pointer ${
                              wizardBudget === bPreset
                      ...
                      <input
                        ...
                        className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none text-slate-900 text-sm focus:border-slate-900 focus:ring-2 focus:ring-slate-900/10 transition-all font-semibold"
                      />
                    </div>
                ...
                <div className="space-y-5 animate-in slide-in-from-right-4 duration-300">
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-bold text-slate-900">Upload Images & Documents</h3>
                        <p className="text-xs text-slate-500">Add site photos, architectural drawings, PDFs, or Drive links</p>
                      </div>
                    </div>
                    <span className="text-[10px] font-black uppercase text-indigo-700 bg-indigo-50 border border-indigo-200 px-2.5 py-1 rounded-full">
                      Max: 10MB Image / 15MB Doc
                    </span>
                  </div>

                  <div
                    className={`
                      border-2 border-dashed rounded-[8px] p-8 text-center transition-all
                  ...
                  >
                    <div className="flex flex-col items-center gap-3">
                      <div className="w-16 h-16 rounded-[8px] bg-slate-100 flex items-center justify-center">
                        <Camera className="w-8 h-8 text-slate-400" />
                      </div>
                      <div>
                        <p className="text-sm font-medium text-slate-700">Drop files here or click to browse</p>
                        <p className="text-xs text-slate-400 mt-0.5">Supports JPG, PNG, WEBP, PDF, DOC, DOCX, ZIP (Max 10MB Images / 15MB Docs)</p>
                      </div>
                      <input
                        type="file"
                        accept="image/*,.pdf,.doc,.docx,.xls,.xlsx,.txt,.zip,.dwg"
                        multiple
                      ...
                      <label
                        ...
                        className={`
                          inline-flex items-center gap-2 px-6 py-2.5 rounded-[6px] text-xs font-bold uppercase tracking-wider cursor-pointer transition-all
                          ${uploadingPhotos ? 'bg-slate-200 text-slate-500 cursor-not-allowed' : 'bg-[#0f2744] hover:bg-[#1e3a8a] text-white shadow-subtle'}
                        `}
                      >
                        {uploadingPhotos ? (
                          <>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-4 gap-3">
                        {wizardPhotos.map((img, idx) => {
                          const isPdf = img.includes(".pdf") || img.includes(".doc") || img.includes(".zip") || img.includes(".docx");
                          return (
                              ...
                              <div className="aspect-square rounded-xl overflow-hidden border border-slate-200 bg-slate-50">
                                {isPdf ? (
                                  <div className="w-full h-full flex flex-col items-center justify-center bg-red-50 p-2 text-center">
                                    ...
                                    <span className="text-[9px] font-bold text-red-700 truncate w-full mt-1">Document</span>
                                  </div>
                                ) : (
                  ...
                  <div className="space-y-2 pt-4 border-t border-slate-100">
                    <div className="flex items-center gap-2">
                      ...
                      <label className="text-xs font-extrabold text-slate-800 uppercase tracking-wider">
                        Google Drive / Dropbox / Cloud Link (Optional)
                      </label>
                    </div>
                    <p className="text-[11px] text-slate-500 font-medium">
                      For large blueprints, CAD files, or shared folders exceeding size limits, paste your Google Drive or Dropbox link here.
                    </p>
                    <input
                      type="url"
                      value={driveLink}
                      ...
                      className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none text-slate-900 text-xs font-mono font-semibold focus:border-indigo-500 focus:bg-white transition"
                    />
                  </div>
                </div>
              )}
              ...
              {step === 4 && (
                <div className="space-y-5 animate-in slide-in-from-right-4 duration-300">
                  ...
                  <div className="bg-slate-50 rounded-2xl border border-slate-200 divide-y divide-slate-200 overflow-hidden">
                    ...
                    {wizardPhotos.length > 0 && (
                      <div className="p-4">
                        <span className="text-xs text-slate-500 block mb-2">Uploaded Attachments ({wizardPhotos.length})</span>
                        <div className="flex gap-2">
                          {wizardPhotos.slice(0, 4).map((img, idx) => (
                            <div key={idx} className="w-12 h-12 rounded-lg overflow-hidden border border-slate-200 bg-slate-100 flex-shrink-0">
                              {img.includes(".pdf") || img.includes(".doc") || img.includes(".zip") ? (
                    ...
                    {driveLink.trim() && (
                      <div className="p-4">
                        <span className="text-xs text-slate-500 block mb-1">Google Drive / Cloud Link</span>
                        <a
                          href={driveLink}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-xs font-mono font-bold text-indigo-600 hover:underline flex items-center gap-1.5 truncate"
                        >
            ...
            <div className="lg:col-span-4 bg-white border border-slate-200 p-6 rounded-lg shadow-sm text-center space-y-4">
              <h3 className="font-extrabold text-xs uppercase tracking-wider text-slate-400">Contractor Schedule</h3>
              <AvailabilityCalendar workerId={worker.id} />
              <p className="text-[10px] text-slate-400 font-semibold leading-relaxed px-1">
                Please check the contractor's blocked dates before submitting your preferred schedule.
              </p>
            </div>
          </div>

          {/* Trust Badge */}
          <div className="mt-6 flex items-center justify-center gap-6 text-xs text-slate-405">
            <div className="flex items-center gap-1.5">
