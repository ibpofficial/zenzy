...
import { recordProfileVisit } from "@/lib/userVisits";
import {
  Home,
  ShoppingBag,
  CheckCircle,
...
function getVideoEmbedUrl(url?: string): { type: "iframe" | "video"; embedUrl: string } | null {
...
export default function WorkerSlugProfilePage({ params }: PageProps) {
  ...
  const { user, userData, role, isAdmin, logout, openAuthModal, updateProfileImage } = useAuth();
  ...
  const [editShowIntroVideo, setEditShowIntroVideo] = useState(false);
    ...
    if (type === "avatar") {
      ...
      try {
        ...
        if (typeof updateProfileImage === "function") {
          ...
          url = await uploadProfileImage(file, worker.id);
        }
        await setDoc(doc(db, "workers", worker.id), { avatar: url, updatedAt: new Date().toISOString() }, { merge: true });
      ...
      try {
        ...
        await setDoc(doc(db, "workers", worker.id), { coverImage: b64 }, { merge: true });
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setAvatarUploading(true);
    try {
      // 1. Compress image to Base64 for instant zero-latency UI preview
      const localBase64 = await compressImageToBase64(file, 400, 0.8);
      setWorker((prev: any) => (prev ? { ...prev, avatar: localBase64 } : prev));

      // 2. Upload to Storage/Base64 and save to Firestore user/worker profile
      let finalAvatarUrl = localBase64;
      if (typeof updateProfileImage === "function") {
        finalAvatarUrl = await updateProfileImage(file);
      } else {
        finalAvatarUrl = await uploadProfileImage(file, worker.id);
      }
      await setDoc(doc(db, "workers", worker.id), { avatar: finalAvatarUrl, updatedAt: new Date().toISOString() }, { merge: true });

      // Update worker state with final URL
      setWorker((prev: any) => (prev ? { ...prev, avatar: finalAvatarUrl } : prev));
      recalculateMyTrust(worker.id);
      alert("✅ Profile picture updated successfully!");
    } catch (err: any) {
      console.error("Avatar upload error:", err);
      alert(`Avatar upload error: ${err?.message || "Failed to update profile picture"}`);
    } finally {
      setAvatarUploading(false);
      if (e.target) e.target.value = "";
    }
  ...
  const handleOpenInfoEdit = () => {
    ...
    setEditShowIntroVideo(worker.showIntroVideo !== false && Boolean(worker.introVideoUrl));
    setEditIntroVideoUrl(worker.introVideoUrl || "");
  ...
  const handleSaveGeneralInfo = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        showIntroVideo: editShowIntroVideo,
        introVideoUrl: editIntroVideoUrl.trim()
      };
  ...
  const handleOpenEditProject = (idx: number) => {
    ...
    setProjMaterials(item.materials || "");
    setProjectShowcaseModalOpen(true);
  };
  ...
  // Load recommended alternative professionals in the same category
  useEffect(() => {
    if (!worker?.category || !worker?.id) return;
    
    const q = query(
      collection(db, "workers"),
      where("category", "==", worker.category),
    ...
    const unsub = onSnapshot(q, (snap) => {
      ...
      snap.forEach((docSnap) => {
        ...
        if (docSnap.id !== worker.id && d.status !== "Busy" && d.availabilityStatus !== "busy") {
    ...
    async function loadProfile() {
      try {
        ...
        if (workerData) {
          ...
          setWorkerId(docId);
          recordProfileVisit(docId, user?.uid);
  ...
  const theme = {
    ...
    card: "bg-white border border-slate-200/80 text-[#0f172a] shadow-[0_2px_16px_rgba(0,0,0,0.06)] rounded-lg hover:shadow-[0_4px_24px_rgba(0,0,0,0.08)] transition-shadow duration-300",
    ...
    input: "bg-white border border-slate-200 text-slate-800 focus:border-slate-900 focus:ring-2 focus:ring-slate-900/20 outline-none rounded-lg transition-all duration-200",
    heading: "font-extrabold tracking-tight text-[#0f172a]",
    pill: "bg-slate-900/10 border border-slate-900/20 text-slate-900 rounded-md font-bold"
  };
  ...
  // Button styles
  const btnPrimary = "bg-slate-900 hover:bg-slate-800 text-white font-extrabold px-6 py-3 rounded-lg transition-all duration-300 shadow-md hover:shadow-lg active:scale-[0.98] inline-flex items-center justify-center gap-2 text-xs uppercase tracking-wider cursor-pointer";
  const btnPrimarySmall = "bg-slate-900 hover:bg-slate-800 text-white font-extrabold px-4 py-2 rounded-md transition-all duration-300 shadow-sm hover:shadow-md active:scale-[0.98] inline-flex items-center justify-center gap-1.5 text-[10px] uppercase tracking-wider cursor-pointer";
  const btnPrimaryOutline = "border-2 border-slate-800 text-slate-800 hover:bg-slate-900 hover:text-white font-extrabold px-6 py-3 rounded-lg transition-all duration-300 active:scale-[0.98] inline-flex items-center justify-center gap-2 text-xs uppercase tracking-wider cursor-pointer";
  const btnWhite = "bg-white hover:bg-slate-50 text-slate-900 font-extrabold px-6 py-3 rounded-lg transition-all duration-300 shadow-sm hover:shadow-md border border-slate-200 active:scale-[0.98] inline-flex items-center justify-center gap-2 text-xs uppercase tracking-wider cursor-pointer";
  ...
  return (
    <div className={`flex flex-col min-h-screen ${theme.bg} transition-colors duration-300 font-sans pt-0`}>
      ...
      {/* RECOMMENDATION STRIP FOR BUSY PROFESSIONALS */}
      {(worker.availabilityStatus === "busy" || worker.status === "Busy" || worker.status === "busy") && recommendations.length > 0 && (
        <section className="max-w-7xl mx-auto w-full px-5 pt-3">
          <div className="relative overflow-hidden bg-rose-50/50 border border-rose-200 p-6 rounded-[2rem] shadow-xs border-l-4 border-l-rose-500 mb-6 animate-fade-up">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
              <div className="space-y-1 text-left flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="w-2.5 h-2.5 rounded-full bg-rose-500 animate-pulse" />
                  <h3 className="font-extrabold text-sm text-slate-800 tracking-tight">Contractor is Currently Busy / Fully Booked</h3>
                </div>
                <p className="text-xs text-slate-500 font-semibold leading-relaxed">
                  To get your project started immediately, consider these alternative top-rated, fully available professionals in the same category:
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full md:w-auto shrink-0">
                {recommendations.map((rec) => (
                  <Link 
                    key={rec.id}
                    href={`/${rec.slug || rec.id}`}
                    className="bg-white border border-slate-200/80 p-3.5 rounded-2xl hover:shadow-md transition flex items-center gap-3 text-left w-full sm:w-60"
                  >
                    <img src={rec.avatar} alt={rec.name} className="w-10 h-10 rounded-xl object-cover shrink-0 border border-slate-100" />
                    <div className="min-w-0 flex-1">
                      <span className="font-extrabold text-xs text-slate-900 block truncate">{rec.name}</span>
                      <div className="flex items-center gap-1 mt-0.5 text-[10px] font-bold text-amber-500">
                        <span>★ {parseFloat(rec.stars || "5.0").toFixed(1)}</span>
                        {rec.trustScore && (
                          <>
                            <span className="text-slate-300">·</span>
                            <span className="text-primary-650">Score: {rec.trustScore.overall}%</span>
                          </>
                        )}
                      </div>
                    </div>
      ...
      <section className="max-w-7xl mx-auto w-full px-5 py-4">
        <div className="bg-white border border-slate-200/60 rounded-2xl p-6 sm:p-7 shadow-sm text-left flex flex-col md:flex-row gap-6 items-start relative">
          ...
          <div className="w-full md:w-64 lg:w-72 h-64 sm:h-72 rounded-2xl overflow-hidden relative shadow-md shrink-0 group border border-slate-100">
            ...
            {canEdit && (
              <>
                <button
                  ...
                  className="absolute top-3 right-3 bg-slate-900/80 hover:bg-slate-900 backdrop-blur-md text-white px-3 py-1.5 rounded-xl flex items-center gap-1.5 transition cursor-pointer shadow-lg text-[10px] font-bold z-10"
                  title="Change Avatar"
                >
                  <Edit className="w-3.5 h-3.5 text-blue-400" />
                  <span>{avatarUploading ? "Uploading..." : "Edit Avatar"}</span>
                </button>
                <input
                  ref={avatarInputRef}
                  type="file"
            ...
            {/* Intro Video button — shown ONLY if showIntroVideo is active and video link exists */}
            {worker.showIntroVideo !== false && Boolean(worker.introVideoUrl?.trim()) && (
              <button
                type="button"
                onClick={() => setIntroVideoModalOpen(true)}
                className="absolute bottom-3 left-3 bg-slate-900/85 hover:bg-slate-900 backdrop-blur-md text-white text-[11px] font-bold px-3.5 py-2 rounded-xl flex items-center gap-2 transition cursor-pointer shadow-lg z-10 border border-white/10"
                title="Play Intro Video"
              >
                <span className="w-5 h-5 rounded-full bg-emerald-500 flex items-center justify-center text-[10px] text-white animate-pulse">▶</span>
      ...
      {/* Trust Score & Availability Schedule Section */}
      <section className="max-w-7xl mx-auto w-full px-5 py-4 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 bg-white border border-slate-200 p-6 rounded-lg shadow-xs flex flex-col justify-start">
          <h3 className="font-extrabold text-xs uppercase tracking-wider text-slate-400 mb-4">Availability Schedule</h3>
      ...
      <section className="max-w-7xl mx-auto w-full px-5 py-2">
        <div className="bg-slate-900 text-white rounded-lg p-6 shadow-lg border border-slate-800 grid grid-cols-2 md:grid-cols-4 gap-4 items-center">
          <div className="flex items-center gap-3">
                ...
                <div className="space-y-8 animate-fade-in">
                  ...
                  <div className={`${theme.card} p-6 sm:p-7 space-y-6`}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 items-center">
                      <div className="sm:col-span-2 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-3 text-xs font-bold text-slate-700">
                          <div className="bg-slate-50 p-3 rounded-lg border border-slate-100 flex items-center gap-2.5">
                            <div className="w-8 h-8 rounded-lg bg-[#1a3a5c]/10 border border-[#1a3a5c]/20 text-[#1a3a5c] flex items-center justify-center shrink-0">
                      ...
                      <div className="bg-[#1a3a5c] text-white rounded-lg p-6 text-center flex flex-col items-center justify-center space-y-2 border border-[#1a3a5c]/80 shadow-xl">
                        <div className="w-12 h-12 rounded-lg bg-white/10 border border-white/20 text-amber-400 flex items-center justify-center">
                  ...
                  <div className={`${theme.card} p-6 sm:p-7 space-y-5`}>
                    ...
                    {(() => {
                      ...
                      if (realServices.length === 0) {
                        return (
                          <div className="bg-slate-50 border border-slate-200/70 rounded-lg p-6 text-center space-y-3">
                            <div className="w-12 h-12 rounded-lg bg-amber-50 text-amber-600 border border-amber-100 flex items-center justify-center mx-auto">
                            ...
                            {canEdit ? (
                              <button
                                onClick={handleOpenAddService}
                                className="bg-[#1a3a5c] hover:bg-[#0f2a4a] text-white font-bold text-xs px-4 py-2.5 rounded-lg transition cursor-pointer shadow-md inline-flex items-center gap-1.5"
                              >
                  ...
                  {(!worker.projectsShowcase || worker.projectsShowcase.length === 0) ? (
                    ...
                    <div className="space-y-8">
                      {worker.projectsShowcase.map((proj: any, idx: number) => {
                        const beforeImg = proj.beforeImages && proj.beforeImages.length > 0
                          ? proj.beforeImages[0]
                          : (proj.images && proj.images.length >= 2 ? proj.images[0] : null);

                        const afterImg = proj.afterImages && proj.afterImages.length > 0
                          ? proj.afterImages[0]
                          : (proj.images && proj.images.length >= 2 ? proj.images[1] : null);

                        const remainingImages = [
                          ...(proj.beforeImages ? proj.beforeImages.slice(1) : []),
                          ...(proj.afterImages ? proj.afterImages.slice(1) : []),
                          ...(proj.images ? proj.images.filter((_: any, i: number) => i > 1 || (i === 1 && !beforeImg)) : [])
                        ].filter(Boolean);

                        let materials = proj.materials || "";
                        if (!materials) {
                          const title = (proj.title || "").toLowerCase();
                          if (title.includes("kitchen")) {
                            materials = "Waterproof Plywood, High-grade Quartz, Acrylic Sheets, Soft-close hinges, Profile LED lights";
                          } else if (title.includes("bath") || title.includes("washroom")) {
                            materials = "Concealed fixtures, Ceramic Tiling, Quartz Countertop, Anti-skid Flooring, Tough Glass partitions";
                          } else if (title.includes("living") || title.includes("hall") || title.includes("bedroom") || title.includes("interior")) {
                            materials = "Teak Wood veneers, Premium Emulsion paint, Profile lighting, Italian Marble, Customized upholstery";
                          } else if (title.includes("paint") || title.includes("wall")) {
                            materials = "Premium Emulsion, Texturized finishes, Anti-damp primers, Protective topcoats";
                          } else {
                            materials = "Premium Materials, High-quality finishes, Durable structural fittings";
                          }
                        }
                        const materialsList = materials.split(",").map((s: string) => s.trim()).filter(Boolean);

                        return (
                          <div key={proj.id || idx} className={`${theme.card} overflow-hidden hover:shadow-xl transition-all duration-300`}>
                            {/* Title Meta Header */}
                            <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex flex-wrap justify-between items-center gap-4">
                              <div>
                                <span className="text-[9px] font-black uppercase text-[#1a3a5c] tracking-widest block">Project Case Study</span>
                                <h4 className="font-extrabold text-lg leading-tight mt-1 text-slate-900">{proj.title}</h4>
                                <span className="text-[11px] font-bold flex items-center gap-1 mt-1.5 text-slate-500">
                              ...
                              {canEdit && (
                                <div className="flex gap-2 shrink-0">
                                  <button
                                    ...
                                    className="bg-white hover:bg-slate-100 border border-slate-200 text-slate-700 px-3.5 py-1.5 rounded-xl transition cursor-pointer text-[10px] font-bold flex items-center gap-1 shadow-sm"
                                  >
                                    <Edit className="w-3 h-3 text-[#1a3a5c]" /> Edit
                                  </button>
                                  <button
                                    onClick={() => handleDeleteProject(idx)}
                                    className="bg-red-50 hover:bg-red-100 text-red-600 p-2 rounded-xl transition cursor-pointer border border-red-100"
                                  >
                            ...
                            {/* Details Grid */}
                            <div className="grid grid-cols-2 md:grid-cols-4 border-b border-slate-200/60 text-center text-xs font-semibold bg-slate-50/30">
                              <div className="p-4 border-r border-b md:border-b-0 border-slate-200/60 space-y-0.5">
                                <span className="text-slate-400 font-bold uppercase text-[9px] block">Budget</span>
                                <span className="text-emerald-600 font-black text-sm block">{proj.cost || proj.budget || "N/A"}</span>
                              </div>
                              <div className="p-4 border-r border-b md:border-b-0 border-slate-200/60 space-y-0.5">
                              ...
                              <div className="p-4 border-r border-slate-200/60 space-y-0.5">
                                ...
                                <span className="text-slate-700 font-black text-sm block">{proj.location || "N/A"}</span>
                              </div>
                              <div className="p-4 space-y-0.5">
                                <span className="text-slate-400 font-bold uppercase text-[9px] block">Client Rating</span>
                                <span className="text-amber-500 font-black text-sm flex items-center justify-center gap-1">
                                  ★ {proj.review?.rating || proj.rating || "5.0"}
                                </span>
                              </div>
                            </div>
                            ...
                            <div className="p-6 space-y-6">
                              ...
                              {beforeImg && afterImg ? (
                                <div className="space-y-3">
                                  <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest block">Before / After Comparison</span>
                                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    {/* Before */}
                                    <div className="relative group overflow-hidden rounded-2xl border border-slate-200/80 aspect-[16/10] bg-slate-100 shadow-sm">
                                      <img src={beforeImg} className="w-full h-full object-cover group-hover:scale-103 transition-transform duration-500" alt="Before Work" />
                                      <div className="absolute top-3 left-3 bg-red-600/90 text-white text-[9px] font-black tracking-widest uppercase px-3 py-1 rounded-full shadow-md backdrop-blur-xs">
                                        Before Work
                                      </div>
                                    </div>
                                    {/* After */}
                                    <div className="relative group overflow-hidden rounded-2xl border-2 border-emerald-400 aspect-[16/10] bg-slate-100 shadow-md">
                                      <img src={afterImg} className="w-full h-full object-cover group-hover:scale-103 transition-transform duration-500" alt="After Work" />
                                      <div className="absolute top-3 left-3 bg-emerald-500 text-white text-[9px] font-black tracking-widest uppercase px-3 py-1 rounded-full shadow-md">
                                        After Work
                                      </div>
                                    </div>
                                  </div>
                                ...
                                <div className="space-y-3">
                                  <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest block">Project Photo</span>
                                  <div className="relative group overflow-hidden rounded-2xl border border-slate-200/80 aspect-[16/10] max-w-lg bg-slate-100 shadow-sm">
                                    <img src={beforeImg || afterImg || undefined} className="w-full h-full object-cover group-hover:scale-103 transition-transform duration-500" alt="Project Showcase" />
                                    <div className="absolute top-3 left-3 bg-[#1a3a5c] text-white text-[9px] font-black tracking-widest uppercase px-3 py-1 rounded-full shadow-md">
                                      Completed Project
                                    </div>
                                  </div>
                                </div>
                              ...
                              {remainingImages.length > 0 && (
                                <div className="space-y-3">
                                  <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest block">Blueprints & Gallery ({remainingImages.length})</span>
                                  <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
                                    {remainingImages.map((img: string, i: number) => (
                                      <div
                                        key={i}
                                        className="relative h-16 sm:h-20 rounded-xl overflow-hidden group cursor-pointer border border-slate-200/60 shadow-sm bg-slate-50"
                                        onClick={() => {
                                          setLightboxImages(remainingImages);
                                          setLightboxIdx(i);
                                          setLightboxOpen(true);
                                        }}
                                      >
                              ...
                              {/* Description Scope & Materials */}
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                                <div className="space-y-2">
                                  <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest block">Scope of Work</span>
                                  <p className="text-xs leading-relaxed text-slate-650 font-medium">{proj.description}</p>
                                </div>
                                <div className="space-y-2.5">
                                  <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest block">Materials Used</span>
                                  <div className="flex flex-wrap gap-1.5">
                                    {materialsList.map((mat: string, mIdx: number) => (
                                      <span key={mIdx} className="px-3 py-1 bg-slate-100 hover:bg-slate-200 border border-slate-200 text-slate-700 text-[10px] font-bold rounded-full transition-colors">
                                        {mat}
                                      </span>
                                    ))}
                                  </div>
                              ...
                              {proj.review && proj.review.clientName && (
                                <div className="p-5 rounded-2xl border border-slate-200/60 bg-slate-50/50 space-y-2.5 shadow-xs">
                                  <div className="flex justify-between items-center text-xs">
                                    <div>
                                      <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block">Client Feedback</span>
                                      <span className="font-extrabold text-slate-800 block mt-0.5">{proj.review.clientName}</span>
                                    </div>
                                    <span className="text-amber-500 font-extrabold flex items-center gap-0.5 bg-amber-500/10 border border-amber-500/20 px-2.5 py-1 rounded-full text-[10px]">
              ...
              <div className="p-4 bg-slate-50 border border-slate-200/80 rounded-2xl space-y-3 text-left">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-black text-slate-800 flex items-center gap-2">
                    ...
                    <span>Show Intro Video on Profile</span>
                  </label>
                  <button
                    type="button"
                    onClick={() => setEditShowIntroVideo(!editShowIntroVideo)}
                    className={`w-11 h-6 rounded-full transition-colors relative p-0.5 cursor-pointer ${
                      editShowIntroVideo ? "bg-emerald-500" : "bg-slate-300"
                    }`}
                  >
                    <div
                      className={`w-5 h-5 rounded-full bg-white shadow-md transform transition-transform ${
                        editShowIntroVideo ? "translate-x-5" : "translate-x-0"
                      }`}
                    />
                  </button>
                </div>
                {editShowIntroVideo && (
                  <div className="space-y-1.5 pt-1">
                    <label className="text-[11px] font-bold text-slate-600 block">
                      Intro Video Link (YouTube or MP4 link)
                    </label>
                    <input
                      type="url"
                      value={editIntroVideoUrl}
                      onChange={(e) => setEditIntroVideoUrl(e.target.value)}
                      placeholder="https://www.youtube.com/watch?v=..."
                      className="w-full bg-white border border-slate-200 rounded-xl px-3.5 py-2 text-xs font-semibold outline-none focus:border-[#1a3a5c]"
                    />
                    <p className="text-[10px] text-slate-400 font-medium">
                      Paste YouTube URL or video file link for client video presentation.
                    </p>
                  </div>
                )}
              </div>
      ...
      {introVideoModalOpen && worker?.introVideoUrl && (
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden max-w-3xl w-full shadow-2xl relative">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-extrabold text-white">{worker.name} — Intro Video</h3>
              </div>
              <button
                onClick={() => setIntroVideoModalOpen(false)}
                className="text-slate-400 hover:text-white p-1.5 rounded-xl hover:bg-white/10 transition cursor-pointer font-bold"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-4 sm:p-6">
              {(() => {
                const embedInfo = getVideoEmbedUrl(worker.introVideoUrl);
                if (!embedInfo) {
                  return <p className="text-red-400 text-xs font-bold text-center py-8">Invalid video URL format.</p>;
                }
                if (embedInfo.type === "iframe") {
                  return (
                    <div className="relative w-full aspect-video rounded-2xl overflow-hidden shadow-lg border border-slate-800 bg-black">
                      <iframe
