...
export default function PublicQuotationPage() {
      ...
      <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
        <main className="flex-1 flex flex-col items-center justify-center p-8 text-center pt-28">
          ...
          <p className="text-xs text-slate-500 max-w-sm mt-2">
            The quotation reference ID &quot;{quoteId}&quot; does not exist or
            has expired.
          </p>
          <Link
            href="/services"
            className="mt-6 bg-[#1a3a5c] text-white px-6 py-3 rounded-xl font-extrabold text-xs uppercase tracking-wider"
          >
  ...
  // Business Identity & Custom Branding
  const brandColor = worker?.brandColor || quote.brandColor || "#0f2b4a";
  const brandLogo = worker?.logo || quote.workerLogo;
  const proName =
    worker?.businessName ||
    worker?.companyName ||
    worker?.name ||
  ...
  const proAddress =
    worker?.serviceArea || worker?.address || quote.workerAddress || "Jaipur, Rajasthan";
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100/50 flex flex-col font-sans text-slate-900 print:bg-white print:p-0">
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto p-4 sm:p-8 pt-6 print:pt-0 print:max-w-none">
        ...
        <div className="relative bg-white rounded-3xl border border-slate-200/90 shadow-2xl overflow-hidden p-6 sm:p-10 space-y-8 print:shadow-none print:border-none print:p-0">
          ...
          {/* Diagonal Status Watermark Overlay */}
          {watermarkText && (
            <div className="absolute inset-0 pointer-events-none flex items-center justify-center overflow-hidden z-10">
              <div
                className={`transform -rotate-45 text-5xl sm:text-7xl font-black uppercase tracking-widest opacity-10 select-none print:opacity-20 border-8 px-8 py-4 rounded-3xl ${watermarkText === "ACCEPTED"
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start border-b border-slate-200/80 pb-8 gap-6">
            <div className="space-y-3">
              <div className="flex items-center gap-4">
                {brandLogo ? (
                  <img
                    src={brandLogo}
                    alt={proName}
                    className="w-16 h-16 rounded-2xl object-cover border border-slate-200 shadow-md shrink-0"
                  />
                ) : (
                  <span
                    className="w-14 h-14 rounded-2xl text-white flex items-center justify-center font-black text-xl shadow-md shrink-0"
                ...
                <div>
                  ...
                  <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight" style={{ color: brandColor }}>
                    {proName}
                  </h1>
                  {gstin && (
              ...
              <div className="text-xs font-medium text-slate-600 space-y-1.5">
                <p className="flex items-center gap-2">
                  <MapPin className="w-3.5 h-3.5 text-slate-400" /> {proAddress}
                </p>
                {proPhone && (
                  <p className="flex items-center gap-2">
                    <Phone className="w-3.5 h-3.5 text-slate-400" /> {proPhone}
                  </p>
                )}
                {licenseNo && (
                  <p className="flex items-center gap-2">
                    <ShieldCheck className="w-3.5 h-3.5 text-emerald-600" /> Reg / License: {licenseNo}
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="bg-slate-50 border border-slate-200/80 p-5 rounded-2xl text-right space-y-2 shrink-0 sm:w-64">
              <div>
                ...
                {(quote.version > 1 || quote.revisionOf) && (
                  <span className="text-[9.5px] font-extrabold bg-indigo-50 text-indigo-700 border border-indigo-200 px-2 py-0.5 rounded-md block mt-1">
                    Revision {quote.version || 2}{" "}
                    {quote.revisionOf ? `of #${quote.revisionOf}` : ""}
                  </span>
                )}
              </div>
              ...
              <div className="pt-1">
                <span
                  className={`inline-block text-[10px] font-black uppercase px-3 py-1 rounded-full ${accepted
          ...
          {(quote.plotArea || quote.projectDuration || quote.structureType) && (
            <div
              className="p-6 rounded-2xl shadow-sm text-white space-y-3"
              style={{ backgroundColor: brandColor }}
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 bg-slate-50/70 p-6 rounded-2xl border border-slate-100">
            <div>
              ...
              {quote.customerPhone && (
                <p className="text-xs text-slate-600 font-semibold mt-0.5 flex items-center gap-1">
                  <Phone className="w-3 h-3 text-slate-400" /> {quote.customerPhone}
                </p>
              )}
              {quote.customerEmail && (
                <p className="text-xs text-slate-500 font-medium flex items-center gap-1">
                  <Mail className="w-3 h-3 text-slate-400" /> {quote.customerEmail}
                </p>
              )}
              {quote.customerAddress && (
                <p className="text-xs text-slate-500 font-medium mt-0.5 flex items-center gap-1">
                  <MapPin className="w-3 h-3 text-slate-400" /> {quote.customerAddress}
                </p>
              )}
            </div>
            ...
            <div>
              ...
              <h3 className="font-extrabold text-sm mt-1" style={{ color: brandColor }}>
              ...
              <p className="text-xs text-slate-600 font-normal leading-relaxed mt-1 line-clamp-2">
                {quote.projectDescription ||
                  quote.notes ||
                  "Itemized breakdown of services, labor, and materials."}
              </p>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="border border-slate-200/80 rounded-2xl overflow-hidden shadow-sm">
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr
                    className="text-white font-extrabold text-[10px] uppercase tracking-wider"
                    style={{ backgroundColor: brandColor }}
                  >
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start gap-6 pt-2">
            <div className="space-y-4 flex-1 text-xs font-semibold text-slate-600 w-full">
              ...
              {/* Numbered Terms & Conditions Clauses */}
            ...
            {/* Right Column: Grand Total & Payment Details Block */}
                ...
                <div className="bg-emerald-50 border border-emerald-200/90 p-4 rounded-2xl space-y-2.5 text-xs">
                  ...
                  {paymentLink && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-[11px] py-2.5 rounded-xl text-center block transition shadow-sm"
                    >
                      Pay Online Now ↗
                    </a>
                  )}
          ...
          {/* E-Signature Seal Display on Accepted Quotes */}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <h4 className="font-extrabold text-sm text-slate-900">
                    Client Signature: &quot;
                    <span className="italic font-serif font-black text-emerald-900">
            ...
            {accepted ? (
              <div className="bg-emerald-50 border border-emerald-200 text-emerald-900 p-6 rounded-2xl text-center space-y-2">
                ...
                <h4 className="font-black text-base">
                  Quotation Authorized & Accepted!
                </h4>
                <p className="text-xs text-emerald-700 font-semibold max-w-md mx-auto">
                  The contractor has received your digital confirmation. Project
                  initiation will begin as scheduled.
                </p>
              </div>
            ) : declined ? (
              <div className="bg-red-50 border border-red-200 text-red-900 p-6 rounded-2xl text-center space-y-2">
                ...
                <p className="text-xs text-red-600 font-medium max-w-md mx-auto">
                  You declined this estimate. Contact the contractor on WhatsApp
                  to request a revised quote.
                </p>
              </div>
            ) : (
              <div className="bg-gradient-to-br from-slate-50 to-slate-100/50 border border-slate-200/80 p-6 rounded-3xl flex flex-col sm:flex-row justify-between items-center gap-4">
                <div>
                  <h4 className="font-black text-sm text-slate-900">
                    Ready to authorize & accept this project quotation?
                  </h4>
                  <p className="text-xs text-slate-500 font-semibold mt-0.5">
                    Click accept to provide your digital signature and notify{" "}
                    {proName}.
                  </p>
                </div>
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto">
                  <button
                    ...
                    className="px-5 py-3 rounded-xl border-2 border-slate-300 text-slate-600 hover:bg-slate-100 font-extrabold text-xs uppercase tracking-wider transition cursor-pointer flex-1 sm:flex-none disabled:opacity-50"
                  >
                    Decline
                  </button>
                  ...
                  <button
                    ...
                    disabled={updatingStatus}
                    className="bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs px-6 py-3 rounded-xl shadow-lg transition cursor-pointer flex items-center justify-center gap-2 flex-1 sm:flex-none active:scale-95 disabled:opacity-50"
                  >
                    <PenTool className="w-4 h-4" />
                    <span>Accept & E-Sign</span>
                  </button>
                </div>
              </div>
            )}
          ...
          {/* Footer Note */}
          <div className="text-center text-[9px] text-slate-400 border-t border-slate-100 pt-4 print:block">
          ...
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-5 animate-fade-in border border-slate-200">
            ...
            <form onSubmit={handleConfirmAcceptQuote} className="space-y-4">
              <p className="text-xs text-slate-600 font-semibold leading-relaxed">
                By signing below, you authorize Quotation #
                {quote.quoteNumber || quote.id.slice(0, 8)} for{" "}
                <strong>₹{grandTotal.toLocaleString("en-IN")}</strong> issued by{" "}
                {proName}.
              </p>
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-extrabold text-slate-900 outline-none focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100"
                />
              </div>
              ...
              <label className="flex items-start gap-2.5 cursor-pointer bg-slate-50 p-3 rounded-xl border border-slate-200">
                ...
                <span className="text-xs text-slate-700 font-semibold leading-snug">
                  I agree to the itemized scope, payment terms, and authorize
                  project execution.
                </span>
              </label>
              ...
              <div className="flex items-center gap-3 pt-2">
                <button
                  type="button"
                  onClick={() => setSignatureModalOpen(false)}
                  className="w-1/2 py-3 rounded-xl border-2 border-slate-300 text-slate-700 font-extrabold text-xs uppercase transition hover:bg-slate-50"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={updatingStatus}
                  className="w-1/2 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs py-3 rounded-xl shadow-md transition flex items-center justify-center gap-1.5 disabled:opacity-50"
                >
