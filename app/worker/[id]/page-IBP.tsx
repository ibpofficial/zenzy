    ...
    <div className="flex flex-col min-h-screen bg-slate-50 font-sans">
      ...
      <main className="max-w-7xl mx-auto w-full px-5 sm:px-8 pt-24 pb-16 flex-grow">
        ...
        <div className="h-56 sm:h-72 rounded-[32px] overflow-hidden relative shadow-float mb-6 bg-slate-200">
          <img
            src={worker?.coverImage || siteConfig?.defaultWorkerBanner || "https://images.unsplash.com/photo-1558979158-65a1eaa14271?auto=format&fit=crop&w=1200&q=80"}
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200/60 shadow-subtle space-y-4">
              <div className="flex justify-between items-center border-b border-slate-100 pb-3">
                <div>
                  <h3 className="font-extrabold text-[16px] text-slate-900 uppercase tracking-wider">Client Reviews</h3>
                  {reviews.length > 0 && (
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <span className="text-xs font-bold text-slate-600">{avgRating} · {reviews.length} reviews</span>
                    </div>
                  )}
                </div>
                <button
              ...
              {reviews.length === 0 ? (
                <p className="text-slate-400 text-sm font-semibold py-4">No reviews yet. Be the first!</p>
              ) : (
                    ...
                    <div key={rev.id} className="pt-5 first:pt-0">
                      <div className="flex justify-between items-start gap-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <span className="font-bold text-[14px] text-slate-900 block">{rev.userName}</span>
                            <span className="text-[10px] text-slate-400 font-semibold">{new Date(rev.createdAt).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</span>
                          </div>
                        </div>
                        <div className="flex">
                      ...
                      <p className="text-slate-600 text-[14px] leading-relaxed mt-3 font-medium">{rev.comment}</p>
                    </div>
                  ))}
                  {reviews.length > visibleReviewsCount && (
                    <div className="pt-5 text-center">
                      <button
                        ...
                        className="bg-slate-50 hover:bg-slate-100 text-slate-700 px-5 py-2.5 rounded-xl text-xs font-bold transition cursor-pointer shadow-subtle border border-slate-100"
                      >
